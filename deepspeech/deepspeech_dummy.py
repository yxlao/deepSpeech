...
utt_lengths = [
    100, 200, 100, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400,
    1500
]
counts = [30, 3, 3, 3, 14, 13, 9, 8, 5, 4, 3, 2, 2, 2, 1]
label_lengths = [
    7, 17, 7, 48, 62, 78, 93, 107, 120, 134, 148, 163, 178, 193, 209
]
...
def _next(batch_size):
    ...
    start_idx = random.randint(0, extra + batch_size *
                               (utt_lengths[-1] - utt_lengths[g_current]) - 1)
...
def inputs(batch_size):
    ...
    return feats.astype(np.float32), idx, vals, s_shape, seq_lens.astype(
        np.int32)
