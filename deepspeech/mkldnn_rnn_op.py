...
class MkldnnRNNCell(BasicRNNCell):
    ...
    def __init__(self,
        ...
        self.model = mkldnn_rnn_ops.MkldnnRNNRelu(
            1, self._num_units, input_size, dropout=0.0)
    ...
    def __call__(self, inputs, state, scope=None, weight_size=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            rnn_weights = _variable_on_cpu("rnn_weights", [self.param_size],
                                           tf.constant_initializer(
                                               1.0 / self.param_size),
                                           self.use_fp16)
            output, output_h = self.model(
                input_data=inputs,
                input_h=state,
                params=rnn_weights,
                is_training=True)
