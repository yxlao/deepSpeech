...
def parse_args():
    ...
    parser.add_argument('--input', type=str, default='ITF_profiling_32.json')
...
def updateGlobalVas(args):
    ...
    if not os.path.exists(
            os.path.join(os.getcwd(), global_vas['output_folder'])):
...
class LayerExeTimeList:
        ...
        for item in stampsList:
            ...
            for i in range(len(stamps)):
                ...
                if self.checkInsideOp(mid, layer_name):
                    ...
                    wallTime_thres += period
                    self.interGapsList.append2Gaps(layer_name, period,
                                                   stamps[i - 1], stamps[i])
    ...
    def printExeTimes(self):
        ...
        for exeTime in self.exeTimeList:
            fp.write("%s, %g, %g\n" %
                     (exeTime['layer_name'], exeTime['wall_time'],
                      exeTime['wall_time_thres']))
        fp.close()
...
class OpsList:
    ...
    def recordBegTime(self, item):
        if self.gmark == False and all(c in item.iterkeys()
                                       for c in time_cond):
            self.gmark = True
    ...
    def groupByLayer(self, input, cmd):
        for key, layer_name in layers_names.iteritems():
            if 'relu' in key or 'ctc' in key:
                ...
                if all(c in cmd for c in opt_inc[layer_name]) and all(
                        c not in cmd for c in opt_exc[layer_name]):
                    self.op.insert(self.layers[layer_name], input, cmd)
                    break
    ...
    def append2List(self, input):
        ...
        if "args" in input.iterkeys() and all(c in input.iterkeys()
                                              for c in time_cond):
            input_name = str(input["args"]["name"])
            self.groupByLayer(input, input_name)
        elif "args" not in input.iterkeys() and all(c in input.iterkeys()
                                                    for c in time_cond):
            self.groupByLayer(input, input_name)
